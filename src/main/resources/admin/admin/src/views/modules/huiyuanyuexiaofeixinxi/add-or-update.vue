<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="月消费额" prop="yuexiaofeie">
          <el-input v-model="ruleForm.yuexiaofeie" 
              placeholder="月消费额" clearable  :readonly="ro.yuexiaofeie"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="月消费额" prop="yuexiaofeie">
              <el-input v-model="ruleForm.yuexiaofeie" 
                placeholder="月消费额" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingbie = true;
          ...
          if(o=='yuexiaofeie'){
            this.ruleForm.yuexiaofeie = obj[o];
	    this.ro.yuexiaofeie = true;
            continue;
          }
        }
      }
    ...
    info(id) {
      this.$http({
        url: `huiyuanyuexiaofeixinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `huiyuanyuexiaofeixinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.huiyuanyuexiaofeixinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
